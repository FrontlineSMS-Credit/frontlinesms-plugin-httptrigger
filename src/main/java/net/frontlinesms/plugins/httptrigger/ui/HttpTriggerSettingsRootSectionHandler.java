...
public class HttpTriggerSettingsRootSectionHandler extends BaseSectionHandler implements UiSettingsSectionHandler, ThinletUiEventHandler {
	...
	protected void init() {
		...
		this.ui.setText(find(UI_COMPONENT_TF_PORT), portNumber);
		this.ui.setSelected(find(UI_COMPONENT_CB_AUTO_START), isAutoStart);
	...
	public void save() {
		...
		properties.setListenPort(Integer.parseInt(this.ui.getText(find(UI_COMPONENT_TF_PORT))));
		properties.setAutostart(this.ui.isSelected(find(UI_COMPONENT_CB_AUTO_START)));
	...
	public List<FrontlineValidationMessage> validateFields() {
		...
		try {
			if (Integer.parseInt(this.ui.getText(find(UI_COMPONENT_TF_PORT))) <= 0) {
